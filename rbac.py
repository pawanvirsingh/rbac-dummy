...
class RBAC(Validation):
	...
	def add_user(self,user_data):
		if self.get(user_data.get("username")):
			print(f"please try diffrent username {role_name}  its Duplicate")
    ...
    def authenticate_user(username,password):
    	...
    	if user_data:
    	   return username.get("password")==password
    	return False
    ...
    def intials(self):
    	pring (f"""hi! you are logged in as admin {self.loggedin_user}
		press 1 for login as another user
		press 2 for create user
		press 3 for edit role""")
    	input_val = input("Please Choose Options")
